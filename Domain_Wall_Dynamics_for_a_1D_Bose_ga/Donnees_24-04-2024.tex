	...
	\section{Simulation GHD}
		...
		\subsection{Méthode 1 : } 
			...
			\begin{enumerate}[label =\Alph*)]
				...
				\item Ajustement de la déformation du bord (\ref{fig:ajustementdeform}):
				...
				\item Selection et Expansion 
					\begin{enumerate}[label =\alph*)]
						\item Selection 
							\begin{itemize}
								\item $x_0 = 19.6 ~\mu m $ (trouvé avec un ajustement gaussien sur {\color{OliveGreen}"expansion 1D $\tau =1~ms$" (\ref{fig:donnes})} ou {\color{orange} (\ref{fig:ajustementdeform}) } ou {\color{blue}(\ref{fig:expansion1})} ) 
								\item $\ell = 24.78 ~ \mu m $ (trouvé en faisant la différence des positions des extremums du gradient des données {\color{OliveGreen}"expansion 1D $\tau =1~ms$" (\ref{fig:donnes})} ou {\color{orange} (\ref{fig:ajustementdeform}) } ou {\color{blue}(\ref{fig:expansion1})} )
							\end{itemize} 
					\end{enumerate}
			\end{enumerate}
