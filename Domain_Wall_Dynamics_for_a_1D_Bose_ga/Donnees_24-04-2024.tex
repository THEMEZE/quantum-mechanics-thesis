...
\begin{document}
    ...
    \begin{enumerate}[label=\alph*)]
        \item "Deformation bord $\tau = 1~ms$" (\ref{fig:donnes}) : Profil longitudinal des données $1~ms$ après la sélection en $x = 0$.
        \item "Deformation bord $\tau = 18~ms$" (\ref{fig:donnes}) : Profil longitudinal des données après $18~ms$ de déformation du bord.
        \item "Expansion 1D $\tau = 1~ms$" (\ref{fig:donnes}) : Profil longitudinal des données après $1~ms$ d'expansion.
        \item "Expansion 1D $\tau = 30~ms$" (\ref{fig:donnes}) : Profil longitudinal des données après $30~ms$ d'expansion.
    \end{enumerate}
  \end{minipage}
	...
	\section{Simulation GHD}
		\begin{figure}[!ht]
			\begin{subfigure}[b]{0.45\textwidth}
        		...
        		\caption{Déformation du bord $\tau = 18 ~ms$ et Ajustement }
        		\label{fig:sub1}
    		\end{subfigure}
    		\vspace{1em}
		...
		\subsection{Méthode 1 : } 
			...
			\begin{enumerate}[label =\Alph*)]
				\item le potentiel chimique est une fonction de la temperature $T$ et la densité $n_p$ : $\mu = f ( T , n_p)$
				\item On extrais la temperature $T$ en faisant un ajustement sur le profil de bord (\ref{fig:donnes})	
			\end{enumerate}
