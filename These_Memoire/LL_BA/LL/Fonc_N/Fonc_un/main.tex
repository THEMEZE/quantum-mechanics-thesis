...
Autrement dit, les seules fonctions propres \( \varphi_{\{\theta\}}(z) \) compatibles avec la structure mathématique imposée par les conditions périodiques sont de la forme :
\(
\varphi_{\{\theta\}}(z) \propto e^{i \theta z}.
\)
Il suffit donc d’étudier ces modes pour capturer l’ensemble des solutions admissibles.
........................
%L’espace de Hilbert \( L^2([0, L]) \), muni de conditions aux limites périodiques, est engendré par la base des exponentielles complexes \( \{ e^{i \frac{2\pi n}{L} z} \}_{n \in \mathbb{Z}} \). Ces fonctions sont les solutions propres de l’opérateur impulsion \( -i\partial_z \), et forment une base orthonormée complète (selon le théorème de Fourier).

%On peut aussi dire que cet espace est engendré par les fonctions de la forme \( e^{\pm i\theta z} \), en posant \( \theta = \frac{2\pi n}{L} \), ce qui correspond à un choix symétrique entre les composantes sinusoïdales directes et inverses.

%Ainsi, toute fonction \( \varphi(z) \in L^2([0,L]) \) vérifiant \( \varphi(z+L) = \varphi(z) \) peut s’écrire comme une combinaison linéaire (ou série de Fourier) de ces fonctions :
%\[
%\varphi(z) = \sum_{n \in \mathbb{Z}} c_n\, e^{i \frac{2\pi n}{L} z}.
%\]
...
\begin{eqnarray}
	\varepsilon(\theta) & = & \frac{\theta^2}{2},% = \frac{2\pi^2 n^2}{L^2},
\end{eqnarray}
