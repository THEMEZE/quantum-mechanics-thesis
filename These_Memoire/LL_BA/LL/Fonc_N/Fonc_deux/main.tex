...
\begin{eqnarray}\label{chap:1:hal.mod.2.part.5}
	- 	\operator{\partial}_Y^2 \tilde{\varphi}_2(Y) + c \operator{\delta} ( Y )\tilde{\varphi}_2(Y) & = & \tilde{E}\tilde{\varphi}_2(Y) 
\end{eqnarray}
...
\begin{eqnarray*}
	\underset{ \epsilon \to 0 }{\lim} \int_{-\epsilon}^{+\epsilon}  - 	\operator{\partial}_Y^2\varphi(Y) + c \operator{\delta} ( Y )\varphi(Y) \, dY  & = & \underset{ \epsilon \to 0 }{\lim}  \int_{-\epsilon}^{+\epsilon}  E d Y ,
\end{eqnarray*}


soit $\varphi'(0^+) - \varphi'(0^-) - c \varphi (  0 )  =  0 $ .En revenant aux coordonnées d'origine, on constate que la fonction d'onde à deux corps $\varphi (z_1, z_2) = \langle 0 \vert \Psi (z_1)\Psi (z_2) \vert \varphi \rangle$ satisfait à cette condition :\\
