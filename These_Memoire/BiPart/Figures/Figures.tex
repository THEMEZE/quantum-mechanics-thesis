...
\begin{tikzpicture}
    ...
    \begin{scope}
		...
		\draw[line width=0.8ex  ,color=\colorFive,  fill = none] 	
			...
			(2.5,9) edge[] node[shift = {(0,0)} , pos=1, right ]{\fontsize{25pt}{19pt}\selectfont \bf $\Pi(\cdot + x_0 /\tau ) $  } (4,9)
		;
		\draw[line width=0.8ex  ,color=\colorOne, dashed ,  fill = none] 	
			plot[smooth] file {data/rho_article_distribution_24-04-2024.table}
			(2.5,8) edge[] node[shift = {(0,0)} , pos=1, right ]{\fontsize{25pt}{19pt}\selectfont \bf $\ell \times \rho ( \cdot + x_0 /\tau ) $  } (4,8)
		;
    \end{scope}
