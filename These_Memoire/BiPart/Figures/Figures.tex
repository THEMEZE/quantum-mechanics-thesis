...
\begin{tikzpicture}
	...
	\newcommand{\GraduationYMm}[2]{
		\foreach \r in { 1 ,... , 9   } {
			\draw ( {-0.25 * 0.5 * (cos(3.14*0.2*\r r)^2 + 1)}    , #1*#2 - #2  +  0.1*#2 *\r ) edge [thick,line width=0.3ex , color = \colorThree  ] ({0.25 * 0.5 * (cos(3.14*0.2*\r r)^2 + 1)} ,#1*#2 - #2 +  0.1*#2 *\r  );
		}
	}
	
\newcommand{\GraduationYCm}[3]{
  \foreach \R in {#1,...,#2} {
    \pgfmathparse{#2[\R-1]}
	...
	\def\listeLabels{{"-un","un", "deux", "trois", "quatre"}}
