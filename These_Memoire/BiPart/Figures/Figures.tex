...
\begin{tikzpicture}
    ...
    \begin{scope}
    	...
    	\draw[line width=0.8ex  ,color=\colorFour ,  fill = none] 	
			...
			(-11.5,10) edge[] node[shift = {(0,0)} , pos=1, right ]{\fontsize{25pt}{19pt}\selectfont \bf $\tau \times n_{\mbox{\tiny GHD}}(\tau \times - x_0 \cdot)$ } (-10,10)
		;
		\draw[line width=0.8ex  ,color=\colorFive,  fill = none] 	
			plot[smooth] file {data/Pi_article_distribution_24-04-2024.table}
			(2.5,9) edge[] node[shift = {(0,0)} , pos=1, right ]{\fontsize{25pt}{19pt}\selectfont \bf $\Pi(\cdot  ) $  } (4,9)
		;
		\draw[line width=0.8ex  ,color=\colorOne, dashed ,  fill = none] 	
			plot[smooth] file {data/rho_article_distribution_24-04-2024.table}
			(2.5,8) edge[] node[shift = {(0,0)} , pos=1, right ]{\fontsize{25pt}{19pt}\selectfont \bf $\ell \ast \rho ( x_0 ) $  } (4,8)
		;
    \end{scope}
	...
	% Axxes 
	\draw[shift = {(-3,-1)}](-10, 0) edge[thick, line width=0.8ex, ->, >=Triangle, color=\colorOne] node[shift = {(0,-1)} , pos=0.5, below]{ \huge $\mu m / m s $ } (10, 0);
    \draw [shift = {(-13,0)}]  (0, -1) edge[thick, line width=0.8ex, ->, >=Triangle, color=\colorOne] node[shift = {(-1,0)} , pos=0.5, above , rotate = 90 ]{ \huge $\theta~(ms/\mu m)$ } (0, 12)
