...
\begin{tikzpicture}
	...
	\pgfmathparse{-4*\echelleXCm} % calcule et arrondit  
    \let\val\pgfmathresult
	\begin{scope}[shift = {(\val,0)}]
	...
	\draw[line width=0.1ex , color = \colorFour , fill = none] 	
		...
		(-12, 0) edge[thick, line width=0.8ex, ->, >=Triangle, color=\colorThree] 
        node[shift = {(0,0)} , pos=1.01, below]{ \huge $x~(\mu m)$ } (9, 0)
        (\val, -1) edge[thick, line width=0.8ex, ->, >=Triangle, color=\colorThree] 
        node[shift = {(0,0)} , pos=0.5, left , rotate = 90 ]{ \huge $n~({\mu m}^{-1})$ } (\val, 12)
