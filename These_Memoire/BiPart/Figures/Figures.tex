...
\begin{tikzpicture}
	\draw[line width=0.1ex , color = \colorFour , fill = none] 	
...
\newcommand{\backgrowngrid}[6]{
	...
	\begin{scope}[]
    	...
    	% Paramètres : #1=xmin, #3=xmax, #2=ymin, #4=ymax
   	 	...
   	 	% Grille principale en cm
    	\draw[very thin, gray] (#1,#2) grid (#3,#4); % Grille de base (1 cm)
    
    	\draw[very thin, gray] (#1,#2) grid[step=0.1] (#3,#4);

    	\draw[thin, black] (#1,#2) edge[thick,line width=0.8ex,->,>=Stealth ] (#2,#3); 
    
    	\foreach \x in {#1,..., #3} {
        	\draw[shift={(\x, #2)}] (0,-0.1) edge[line width=0.8ex] node[pos = -1 , below] {\x * #5 } (0,0.1); % Lignes verticales en mm
    	}

    	\draw[thin, black] (#1,#2)edge[thick,line width=0.8ex,->,>=Stealth ] (#1,#4); 
    
    	\foreach \y in {#2,..., #4} {
        	\draw[shift={(#1, \y)}] (-0.1 , 0 ) edge[line width=0.8ex] node[pos = -1 , left ] {\y * #6} (0.1 , 0); % Lignes verticales en mm
    	}
...
\begin{scope}
%\clip (-11 , -3) rectangle (11 , 5) ;
\GraduationYCm{0}{6}{\listeLabelsY}{\echelleYCm}
%\drawgrid{-11}{15}{-11}{11}
%\draw[line width=2.5ex , color = red ] (-11 , -3) rectangle (11 , 5) ;
%\path (-11 , -3) rectangle (11 , 5);	
\end{scope}

% Définition de l’échelle
\def\echelleX{50}
\pgfmathsetmacro{\echelleXCm}{18/7}
...
\draw[line width=2.5ex , color = red ] (-11 , -3) rectangle (11 , 12) ;


\backgrowngrid{-11}{-3}{11}{12}{\echelleX}{\echelleY}
