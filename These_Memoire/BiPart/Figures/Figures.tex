...
\begin{tikzpicture}
	...
	%\clip[scale = 0.95 , decorate, decoration={random steps, segment length=0.1 cm, amplitude=0.2cm}](#1 , #2 ) rectangle ( #3 , #4 ) ;
	\clip[](-14 , -3) rectangle (14 , 12) ;
	
	% Définition de l’échelle
	%\def\echelleY{10}
	\pgfmathsetmacro{\echelleYCm}{9/3}
	%\def\echelleX{50}
	\pgfmathsetmacro{\echelleXCm}{20/4}
    ...
    \begin{scope}
    	...
    	\draw[line width=0.8ex  ,color=\colorFive ,  fill = none] 	
			plot[] file {data/donnee_article_asymetrie_24-04-202.table}
			(-10.5,10) edge[] node[shift = {(0,0)} , pos=1, right ]{\fontsize{19pt}{19pt}\selectfont \bf Profil de bord } (-9,10)
		;
		\draw[line width=0.8ex  ,color=\colorSix ,  fill = none] 	
			plot[] file {data/donnee_2_article_asymetrie_24-04-202.table}
			(-10.5,9) edge[] node[shift = {(0,0)} , pos=1, right ]{\fontsize{19pt}{19pt}\selectfont \bf Ajustement : T = 560~nK } (-9,9)
		;
    \end{scope}
	...
	% Axxes 
	\draw[shift = {(0,-2)}](-13, 0) edge[thick, line width=0.8ex, ->, >=Triangle, color=\colorOne] node[shift = {(0,-1)} , pos=0.5, below]{ \huge $x~(\mu m)$ } (13, 0);
    \draw   (-13, -1) edge[thick, line width=0.8ex, ->, >=Triangle, color=\colorOne] node[shift = {(-1,0)} , pos=0.5, above , rotate = 90 ]{ \huge $n~({\mu m}^{-1})$ } (-13, 12)
