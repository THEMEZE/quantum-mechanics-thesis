...
Nous préparons d’abord un gaz homogène. Ensuite, nous supprimons brutalement une moitié du nuage. Cela crée une frontière nette entre deux zones : une avec des atomes, l’autre vide. Cette configuration correspond à ce qu’on appelle un « quench bipartite ».

\begin{figure}[H]
	\centering
	\includegraphics[width=0.5\textwidth , page = 4 ]{Shema.pdf}
	\caption{a) Facteur d'occupation initiale $\nu( x , \theta ) = \nu_0(\theta)$  correspondant à l'équilibre thermique à la température $T = 560 ~nK$ à une densité spatial linéaire $n_p= 56 ~{\mu m}^{-1}$ et donc à potentielle chimique $\mu = 65~nK$. b) Densité spatial linéaire $n c) Facteur d'occuation initiale $\nu_0(\theta)$ correspondant au diagramme $a)$. }
	\label{fig:grap.A}
\end{figure}
