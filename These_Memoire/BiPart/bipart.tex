...
(c) Distribution de rapidité après sélection, $\Pi(\theta) = \int \rho(x,\theta ; \tau)\,\mathrm{d}x$, invariante sous l'évolution, représentée en rouge ; la distribution localement résolue en $x(s;\tau = O^{+})$, $\rho(x(s;\tau = O^{+}), \theta ; \tau = 0^+)$, est représentée en pointillés. 
