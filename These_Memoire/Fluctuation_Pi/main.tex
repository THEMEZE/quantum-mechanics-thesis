...
\section{Principe de fluctuation-réponse dans un état diagonal}
...
On a vus  que l’espérance d’une observable $\operator{\mathcal{Q}}^{(\mathcal{S})}[w_1]$ dans le GGE , noté dans le chapitre \ref{} par $\langle \operator{\mathcal{Q}}^{(\mathcal{S})}[w] \rangle_{\operator{\rho}_{\mathrm{GGE}}^{(\mathcal{S})}[w]}$ que pour soucis d'aléger dans lé résonement on notéra $\langle \operator{\mathcal{Q}}[w_1] \rangle$, et j'enlève les $(\mathcal{S})$, le caractère locale étant maintenat inplicite , est donnée par :
\begin{eqnarray*}
	\langle \operator{\mathcal{Q}}[w_1] \rangle = \mathrm{Tr} \left[ \operator{\rho}_{\mathrm{GGE}}[w]\, \operator{\mathcal{Q}}[w_1] \right]
= \frac{1}{Z[w]}\, \mathrm{Tr} \left[ e^{-\operator{\mathcal{Q}}[w]}\, \operator{\mathcal{Q}}[w_1] \right] =  -  \left . \frac{ \delta \ln Z [w] }{ \delta w_1  } \right|_{w}.	
\end{eqnarray*}

La dérivée fonctionnelle de l'espérance de la charge $\operator{\mathcal{Q}}[w_1]$ par rapport au potentiel conjugué $w_2$ est donnée par :
\begin{eqnarray*}
	\chi[w_1, w_2] \doteq - \left.\frac{\delta \langle \operator{\mathcal{Q}}[w_1] \rangle}{\delta w_2} \right )_w =  \left . \frac{ \delta^2 \ln Z [w] }{ \delta w_1 \delta w_2 } \right|_{w}= \langle \operator{\mathcal{Q}}[w_1] \operator{\mathcal{Q}}[w_2] \rangle - \langle \operator{\mathcal{Q}}[w_1] \rangle \langle \operator{\mathcal{Q}}[w_2] \rangle .	
\end{eqnarray*}
...
\begin{eqnarray*}
\operator{\mathcal{Q}}[w] = L\int d\theta w(\theta) \operator{\rho}(\theta)
\end{eqnarray*}
où $\operator{\rho}(\theta)$ est l'opérateur densité spectale , agissant comme :
\(
...
\(
	\left.\frac{\delta}{\delta w_2} \left( \frac{A[w]}{Z[w]} \right) \right |_{w} = \frac{1}{Z[w]}\, \left.\frac{\delta A[w]}{\delta w_2} \right|_{w} - \frac{A[w]}{Z[w]^2}\, \left.\frac{\delta Z[w]}{\delta w_2}\right|_{w} ,
\)
avec $A[w] \equiv \mathrm{Tr}[e^{-\operator{\mathcal{Q}}[w]} \operator{\mathcal{Q}}[w_1]]$, on obtient :
\begin{eqnarray*}
	\left . \frac{\delta \langle \operator{\mathcal{Q}}[w_1] \rangle}{\delta w_2} \right )_w
	= \frac{1}{Z[w]}\, \frac{\delta A[w]}{\delta w_2}
	- \langle \operator{\mathcal{Q}}[w_1] \rangle \cdot \frac{1}{Z[w]}\, \frac{\delta Z[w]}{\delta w_2}.
\end{eqnarray*}

\subparagraph{Dérivées explicites.}
Utilisons la propriété démontrée précédemment :
\(
	\frac{\delta \operator{\mathcal{Q}}[w]}{\delta w_2} = \operator{\mathcal{Q}}[w_2]	.
\)
En notant que la dérivée d’une exponentielle diagonale est simple :
\(
\frac{\delta}{\delta w_2} e^{-\operator{\mathcal{Q}}[w]} = - \operator{\mathcal{Q}}[w_2]\, e^{-\operator{\mathcal{Q}}[w]},
\)
on obtient :
\begin{eqnarray*}
\frac{\delta A[w]}{\delta w_2} = - \mathrm{Tr}\left[ \operator{\mathcal{Q}}[w_2]\, e^{-\operator{\mathcal{Q}}[w]} \operator{\mathcal{Q}}[w_1] \right],
\quad
\frac{\delta Z[w]}{\delta w_2} = - \mathrm{Tr}\left[ \operator{\mathcal{Q}}[w_2]\, e^{-\operator{\mathcal{Q}}[w]} \right].
\end{eqnarray*}

\subparagraph{Regroupement.}
En regroupant les termes, on trouve :
\begin{eqnarray*}
	\chi[w_1, w_2]&  \doteq & - \left.\frac{\delta \langle \operator{\mathcal{Q}}[w_1] \rangle}{\delta w_2} \right )_w  \\
&=& -\frac{1}{Z[w]}\, \mathrm{Tr}\left[ \operator{\mathcal{Q}}[w_2]\, e^{-\operator{\mathcal{Q}}[w]}\, \operator{\mathcal{Q}}[w_1] \right]
+ \langle \operator{\mathcal{Q}}[w_1] \rangle \cdot \frac{1}{Z[w]}\, \mathrm{Tr}\left[ \operator{\mathcal{Q}}[w_2]\, e^{-\operator{\mathcal{Q}}[w]} \right] \\
&=& \langle \operator{\mathcal{Q}}[w_2]\, \operator{\mathcal{Q}}[w_1] \rangle - \langle \operator{\mathcal{Q}}[w_1] \rangle\, \langle \operator{\mathcal{Q}}[w_2] \rangle.
\end{eqnarray*}
...
\subparagraph{Corrélation spectrale.}
On peut alors exprimer la susceptibilité comme :
\begin{eqnarray*}
\chi[w_1, w_2] = L^2\iint d\theta\, d\theta'\, w_1(\theta)\, w_2(\theta')\, C(\theta, \theta'),
\end{eqnarray*}
où la fonction de corrélation spectrale $C$ est définie par :
\begin{eqnarray*}
C(\theta, \theta') \doteq \langle \delta \operator{\rho}(\theta)\, \delta \operator{\rho}(\theta') \rangle
= \langle \operator{\rho}(\theta) \operator{\rho}(\theta') \rangle - \langle \operator{\rho}(\theta) \rangle \langle \operator{\rho}(\theta') \rangle.
\end{eqnarray*}
La matrice $\chi[w_1, w_2]$ s’interprète donc comme la covariance de la densité spectrale projetée sur les fonctions test $w_1$ et $w_2$.

\medskip
Un cas particulier d’intérêt est la susceptibilité d’une seule charge :
\begin{eqnarray*}
\chi[w] \equiv \chi[w, w] = \mathrm{Var}(\operator{\mathcal{Q}}[w]) = \langle \operator{\mathcal{Q}}[w]^2 \rangle - \langle \operator{\mathcal{Q}}[w] \rangle^2
= \iint d\theta\, d\theta'\, w(\theta)\, w(\theta')\, C(\theta, \theta').
\end{eqnarray*}

\section{Passage au régime thermodynamique}

Dans l’approximation thermodynamique, l’opérateur de densité spectrale $\hat{\rho}(\theta)$ est remplacé par sa valeur moyenne macroscopique $\rho_{\mathrm{eq}}(\theta)$, représentant la densité continue de quasi-particules dans l’état d’équilibre.

Dans le cadre du GGE continu, l’état macroscopique est entièrement déterminé par une densité spectrale $\rho(\theta)$ qui maximise l’entropie de Yang-Yang $\mathcal{S}_{\mathrm{YY}}[\rho]$, sous la contrainte d’une charge généralisée fixée. Le poids spectral $w$ est fixé. On a vus dans les chapitre \ref{??} que la condition d’équilibre s’écrit alors :
\(
...
\begin{itemize}
  ...
  \[
  \chi(\theta, \theta') = \frac{\delta \rho_{\mathrm{eq}}(\theta)}{\delta w(\theta')}.
  \]
  ...
  \[
  \chi(\theta, \theta') = \langle \delta \hat{\rho}(\theta)\, \delta \hat{\rho}(\theta') \rangle.
  \]
  \item Dans la limite thermodynamique, où $\hat{\rho}(\theta) \to \rho(\theta)$, on peut omettre les chapeaux :
  \[
  \chi(\theta, \theta') = \langle \delta \rho(\theta)\, \delta \rho(\theta') \rangle.
  \]
\end{itemize}
...
\[
\chi(\theta, \theta') = \left.\frac{\delta^2 \ln Z[w]}{\delta w(\theta)\, \delta w(\theta')} \right|_{w} = -\frac{\delta \rho_{\mathrm{eq}}(\theta)}{\delta w(\theta')} = -\left( L \left. \frac{\delta^2 \mathcal{S}_{YY}[\rho]}{\delta \rho(\theta)\, \delta \rho(\theta')}\right|_{\rho = \rho_{\mathrm{eq}}} \right)^{-1}
 = - \left( L \mathcal{H}^{(\mathcal{S}_{\mathrm{YY}})} \right)^{-1} (\theta, \theta') = \langle \delta \rho(\theta)\, \delta \rho(\theta') \rangle = C(\theta, \theta').
\]
...
\subsection{Formalisme gaussien}
...
En injectant cette dernier équation et \ref{chap:fluctu:eq:hessienner.inv.1} , dans (\ref{chap:fluctu:eq:fluctuations}) , il vient que une éqution implicite:
\begin{eqnarray*}
...
\subsection{Lien entre dérivée fonctionnelle et réponse linéaire aux facteurs de Lagrange}
...
\[
\operator{Q}_i = \mathcal{Q}[f_i] = L \int d\theta\, f_i(\theta)\, \hat{\rho}(\theta),
\]
...
\[
\left. \frac{\partial \langle \operator{Q}_i \rangle }{ \partial \beta_j } \right|_{\beta_{k \ne j}} = \int d\theta'\, \left. \frac{\delta \langle \operator{Q}_i \rangle }{ \delta w(\theta') } \right|_{w} \cdot \frac{\partial w(\theta')}{\partial \beta_j}.
\]
Comme \( \partial w(\theta') / \partial \beta_j = f_j(\theta') \), on obtient :
\[
\left. \frac{\partial \langle \operator{Q}_i \rangle }{ \partial \beta_j } \right|_{\beta_{k \ne j}} = \int d\theta'\, \left. \frac{\delta \langle \operator{Q}_i \rangle }{ \delta w(\theta') } \right|_{w} f_j(\theta').
\]

\paragraph{Expression de la dérivée fonctionnelle.}
On écrit :
\[
\left. \frac{\delta \langle \operator{Q}_i \rangle }{ \delta w(\theta') } \right|_{w} = L \int d\theta\, f_i(\theta)\, \left. \frac{\delta \langle \operator{\rho}(\theta) \rangle }{ \delta w(\theta') } \right|_{w} = L \int d\theta\, f_i(\theta)\, \chi(\theta, \theta'),
\]
où \( \chi(\theta, \theta') := \left. \frac{\delta \langle \operator{\rho}(\theta) \rangle }{ \delta w(\theta') } \right|_{w} \) est la {\em matrice de susceptibilité spectrale locale}.
...
\[
\boxed{
\chi_{ij} := - \left. \frac{\partial \langle \operator{Q}_i \rangle }{ \partial \beta_j } \right|_{\beta_{k \ne j}} = - \left. \frac{\delta \langle \mathcal{Q}[f_i] \rangle }{ \delta f_j } \right|_{w} \doteq  \chi[f_i , f_j ] = L^2 \iint d\theta\, d\theta'\, f_i(\theta)\, \chi(\theta, \theta')\, f_j(\theta')
}
\]

Ce résultat relie :
\begin{itemize}
  \item la réponse linéaire de \( \langle Q_i \rangle \) à une perturbation \( \beta_j \),
  \item la dérivée fonctionnelle de l’observable \( \mathcal{Q}[f_i] \) par rapport à sa fonction test,
  \item la projection de la matrice de susceptibilité spectrale locale \( \chi(\theta, \theta') \) sur \( f_i, f_j \).
\end{itemize}
...
\[
\chi_{\operator{Q},\operator{Q}} = \left . \frac{\partial \langle \operator{Q} \rangle}{\partial (\beta \mu)} \right|_\beta  = L^2 \iint d\theta\, d\theta'\, \chi(\theta, \theta').
\]
...
\[
\chi_{\operator{H},\operator{H}} := -\left.\frac{\partial \langle \operator{H} \rangle}{\partial \beta} \right|_{\beta \mu}  = L^2 \iint d\theta\, d\theta'\, \varepsilon(\theta)\, \chi(\theta, \theta')\, \varepsilon(\theta').
\]
...
%Pour toute charge \( \mathcal{Q}[f] = L \int f(\theta)\, \hat{\rho}(\theta)\, d\theta \), on a :
%\[
%\boxed{
