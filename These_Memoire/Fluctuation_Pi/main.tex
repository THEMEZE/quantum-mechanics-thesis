...
Considérons un système intégrable unidimensionnel, homogène, de longueur \(L\). Comme établi dans le chapitre~\ref{chap:relaxation}, sa relaxation vers un état stationnaire est décrite par un \textit{Generalized Gibbs Ensemble} (GGE). La moyenne d’une observable locale \(\langle\operator{\mathcal{O}}\rangle\) se formule alors comme une intégrale fonctionnelle sur les distributions de rapidité~\cite{YangYang1969} :
\begin{eqnarray}
	\underset{\mbox{\tiny therm.}}{\lim}\langle \operator{\mathcal{O}} \rangle_{GGE} & = & \frac{\int \mathcal{D} \rho \; e^{L (\mathcal{S}_{YY}[\rho] - \mathcal{W}[\rho])} \, \langle\mathcal{O} \rangle_{[\rho]}}{\int \mathcal{D} \rho \; e^{L (\mathcal{S}_{YY}[\rho] - \mathcal{W}[\rho])}}, \label{chap:fluctu:eq:ensemble_average}
\end{eqnarray}
avec $\langle\mathcal{O} \rangle_{[\rho]}$ la valeur de l’observable dans un état propre caractérisé par la distribution de rapidité $\rho$, $\mathcal{S}_{YY}$ l'entropie de Yang-Yang \eqref{chap.2.entropi.int} et $\mathcal{W}$ l'énergie généralisé par unité de longueur \eqref{chap.2.W.int}.
...
\begin{eqnarray}
	\underset{\mbox{\tiny therm.}}{\lim}\langle \operator{\mathcal{O}} \rangle_{GGE} & \approx & \langle\mathcal{O} \rangle_{[\rho_{eq}]}	
...
%On considère deux observable $\operator{\mathcal{O}}$ et $\operator{\mathcal{R}}$ , et on s’intéresse à leur corrélation connectée (ou fonction de corrélation à deux points) :
%Pour accéder aux fonctions de corrélation à deux points,
...
Sous l’approximation gaussienne autour du point col, la covariance des fluctuations est donnée par :
\begin{eqnarray}
...
\begin{equation}
  C_{\operator{\mathcal{A}},\operator{\mathcal{B}}}^{[\rho]}
  \;=\;
  \bigl\langle
    \bigl(\operator{\mathcal{A}}-\langle\operator{\mathcal{A}}\rangle_{[\rho]}\bigr)
    \bigl(\operator{\mathcal{B}}-\langle\operator{\mathcal{B}}\rangle_{[\rho]}\bigr)
  \bigr\rangle_{[\rho]}
  \;=\;
  \langle\operator{\mathcal{A}}\operator{\mathcal{B}}\rangle_{[\rho]}
  - \langle\operator{\mathcal{A}}\rangle_{[\rho]}\,\langle\operator{\mathcal{B}}\rangle_{[\rho]}.
  \label{eq:corr-conn}
\end{equation}
...
\begin{eqnarray*}
	\langle\operator{\mathcal{A}}\rangle_{[\rho]}= \underbrace{\int d\theta\,a(\theta)\,\rho_{eq}(\theta)}_{\langle\operator{\mathcal{A}}\rangle_{[\rho_{eq}]}}  +  \underbrace{\int d\theta\,a(\theta)\,\delta \rho(\theta)}_{\delta\langle\operator{\mathcal{A}}\rangle_{[\rho_{eq}]}}	
\end{eqnarray*}

où \(a(\theta)\)  le densités spectrales associées à la valeur moyenne de le l'operateur $\operator{\mathcal{A}}$ associé à la distribution à l'équilibre,  $\langle\operator{\mathcal{A}}\rangle_{[\rho_{eq}]}$ .

\subsection*{Formalisme gaussien}

Dans la limite thermodynamique,
l’intégrale fonctionnelle du GGE est dominée
par les fluctuations gaussiennes autour du point selle
\(\rho_{\mathrm{eq}}\) :
\[
\Phi[\rho]
\simeq
...
\mathcal{H}=
\left.\frac{\delta^{2}
      (\mathcal{S}_{YY}-\mathcal{W})}{\delta\rho\,\delta\rho}\right|_{\rho_{\mathrm{eq}}}
\)
est le \textit{hessien} de l’action effective.

L’intégrale gaussienne fournit
\begin{equation}
  \bigl\langle
    \delta\rho(\theta)\,\delta\rho(\theta')
  \bigr\rangle
  =
  -\frac{1}{L}\,\mathcal{H}^{-1}(\theta,\theta').
  \label{eq:cov-rho}
\end{equation}
En reportant \eqref{eq:cov-rho} dans \eqref{eq:corr-conn},
...
\begin{equation}
  ...
  -\frac{1}{L}
  \iint d\theta\,d\theta'\;
  a(\theta)\,\mathcal{H}^{-1}(\theta,\theta')\,b(\theta').
  \label{eq:2point}
\end{equation}
