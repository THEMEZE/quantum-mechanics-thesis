...
\begin{eqnarray}
    \ln \# \mbox{microstates.} & = & \sum_a^{\theta-\mbox{\tiny tranches}} \ln \# \mbox{conf.}(\theta_a), \\
...
\begin{eqnarray}
	\mathcal{W}[w] & = & \sum_{a = 1}^N  w(\theta_a)	 ~=~ L \sum_a^{\theta-\mbox{\tiny tranches}}	 w(\theta_a) \rho(\theta_a) \delta \theta.
\end{eqnarray}
