...
\[
\operator{H}_{\text{int}} : \ket{d, N+1} \longleftrightarrow \ket{e_i, N},
\]
où \( \ket{f, N+1} \) est un état fondamental avec \( N+1 \) photons, et \( \ket{e_i, N} \) un état excité \( e_i \) avec \( N \) photons.
...
Dans cette base, seule la composante $D_q$ correspondant au changement $m_L \to m_L' = m_L + q$ est active :
\begin{eqnarray*}
...
\end{eqnarray}
Le coefficient de Clebsch-Gordan impose $m_L' = 0$, ce qui implique que seule la composante $q = 0$ (polarisation rectiligne) contribue à cette transition spécifique :
\begin{eqnarray}
\Rightarrow \bra{L' = 1, m_L'=0} \operatorvec{D} \cdot \vec{u}_{q=0} \ket{L = 0, m_L = 0}
= d_{\scriptstyle 5S \leftrightarrow 5P} \cdot \delta_{m_L', m_L + q} .
\end{eqnarray}
